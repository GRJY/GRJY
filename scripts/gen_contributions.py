...
def motion_css():
    # 4.2s cycle — solid 0-28% (~1.2s), dust by 55%, gone until 68%, back by 97%.
    # The grid is never still for long; that is the point of asking for it.
    css = ["""
  .g { animation-duration: 4.2s; animation-iteration-count: infinite;
       transform-box: fill-box; transform-origin: center; }
  .core { animation-name: core; }
  @keyframes core {
    0%, 28%    { opacity: 1; transform: none; }
    30%        { opacity: 1; transform: scale(1.16); }
    38%        { opacity: 0; transform: scale(.9); }
    68%        { opacity: 0; transform: scale(.9); }
  ...
  @keyframes ash {
    ...
    55%       { opacity: 0; transform: translate(17px,-13px) scale(.22); }
    68%       { opacity: 0; transform: translate(17px,-13px) scale(.22);
...
""".replace("BURST_C", BURST).replace("RETURN_C", RETURN)]
  ...
  @keyframes f{i} {{
    ...
    30%      {{ opacity: 1; transform: translate({dx * 0.05:.1f}px,{dy * 0.05:.1f}px)
                           rotate({rot * 0.06:.1f}deg); }}
    58%      {{ opacity: 0; transform: translate({dx}px,{dy}px) rotate({rot}deg) scale(.26); }}
    68%      {{ opacity: 0; transform: translate({dx}px,{dy}px) rotate({rot}deg) scale(.26);
                animation-timing-function: {RETURN}; }}
    97%, 100% {{ opacity: 1; transform: none; }}
  }}""")

    for i, (dx, dy, lingers) in enumerate(MOTES):
        gone = 78 if lingers else 62
        peak = ".55" if lingers else ".85"
        css.append(f"""
  .m{i} {{ animation-name: m{i}; }}
  @keyframes m{i} {{
    0%, 29%  {{ opacity: 0; transform: none;
                animation-timing-function: {BURST}; }}
    33%      {{ opacity: {peak}; transform: translate({dx * 0.1:.1f}px,{dy * 0.1:.1f}px); }}
...
def build(days, theme, total, since_date, private_repos):
    ...
    for w in range(WEEKS):
        for d in range(7):
            ...
            wave = int(w * 15 + d * 4 + rnd(w, d, 7) * 110)
